              ...
              url:cn + "PMedical&Tipo="+t+ "&Rol="+r+"&Nom="+nom, success: function (result) {    
                ///console.log(result);
                  ...
                  for (var i = 0; i < data.length; i++) {
                      ...
                      html = html + '<article class="col-6 col-xxl-4"><div class="card h-100 overflow-hidden"><div class="card-body p-0 d-flex flex-column justify-content-between"><div><div class="hoverbox text-center"><img class="w-100 h-100 fit-cover" src='+img+' alt=""></div><div class="p-3"><h5 class="fs-0 mb-2"><a onclick="modal(\''+ img +'|'+data[i].name +'|' + data[i].id+'\')" class="text-dark">'+ n +'</a></h5><h5 class="fs-0"><a>'+ data[i].descr +'</a></h5></div></div><div class="row g-0 mb-3 align-items-end"><div class="col ps-3"><h4 class="fs-1 text-warning d-flex align-items-center"><span>$ ' + data[i].price +'</span></h4></div><div class="col-auto pe-3"><a id=\"'+data[i].id +'|'+ n +'\" style="cursor:pointer" name=\"'+data[i].price+'\" onclick="addcarro(this)" class="btn btn-sm btn-primary d-lg-block mt-lg-2" href="#!"><span class="fas fa-cart-plus"></span></a></div></div></div></article>';
