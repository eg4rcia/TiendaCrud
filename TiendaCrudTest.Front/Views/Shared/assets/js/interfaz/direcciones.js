function verDomicilios(){
    $("#loading").show();
      ...
      var filtro = localStorage.getItem("idu");
        $.ajax({
          type: "GET",
          url: cn + "SeleccionarDomicilios&id="+filtro, 
...
if(data[i].activo == true){
    ...
    activo = "";
            ...
            html = html + '<div class="col-md-6 mb-3 mb-md-0"><div class="form-check mb-0 custom-radio radio-select"><input class="form-check-input" id="address-'+data[i].iddomicilio+'" onclick="selectDestino(this.id)" type="radio" name="clientName" '+activo+' ><label class="form-check-label mb-0 fw-bold d-block" for="address-'+data[i].iddomicilio+'">'+data[i].calle+'<span class="radio-select-content"><span> '+data[i].nexterior+' '+data[i].calle+',<br>'+data[i].estado+',<br>'+data[i].municipio+' '+data[i].colonia+' '+data[i].codigopostal+'<span class="d-block mb-0 pt-2">'+data[i].telefono+'</span></span></span></label><a class="fs--1" href="#!">Edit</a></div></div>';
         ...
         activo="";
          ...
          document.getElementById("divDomicilios").innerHTML = html + '<br><br>';
          $("#loading").hide();
          },
      ...
      function selectDestino(valor){
        ...
        $.ajax({
            type: "GET",
            crossDomain: true,   
            url: cn + "ElegirDomicilios&id="+filtro+"&iddomicilio="+valor[1],
            success: function (result) {
                console.log(result);
            },
            error: function (jqXmlHttpRequest, textStatus, errorThrown) {
